...
class ResNet(nn.Module):
	...
	def __init__(self, block, block2, layers, num_segments, flow_estimation, num_classes=1000, zero_init_residual=False):
		...
		self.sparse_transformer = SparseTransformer(2048,16,4,4)
		...
		self.stack_diff_motion = nn.Sequential(
			nn.Conv2d(12,24,3,2,1,bias=False),
			nn.ReLU(inplace=True),
			nn.BatchNorm2d(24),
			nn.Conv2d(24,64,3,1,1,bias=False,groups=4),
			nn.ReLU(inplace=True),
			nn.BatchNorm2d(64),
